...
import { aggregateFaces, summariseFaceDraft, summariseFeatures } from './faces.js';
...
export function analyseMesh(geom, opts = {}) {
  ...
  const faces = geom.faceGroups
    ? aggregateFaces(geom, { triAreas, triFNorm, triPullDot, triDraft, triFaceSide, triCentroid, minDraft, isTwoPiece })
    : null;
  ...
  return {
    ...
    /* The cylindrical features: fillets and rounds to judge, bores and bosses
       to report. Null on a mesh source, which carries no faces to fit. */
    features: faces ? summariseFeatures(faces) : null,
